...
class User extends CI_Controller
{
    ...
    public function user_supp()
    {
        $data['user'] = $this->user_m->list_supp()->result();
        $data['supplier'] = $this->supplier_m->get()->result();
        // var_dump($data['supplier']);
        $this->template->load('template', 'user/user_data_supp', $data);
    }
    ...
    public function cek_username_sup()
    {
        $username = $this->input->post('data');
        $cek_data = $this->user_m->cek_data_sup($username)->row_array();
        $return_data = ($cek_data) ? "ADA" : "TIDAK ADA";
    ...
    public function save_sup()
    {

        $post = $this->input->post();
        $this->user_m->save_sup($post);

        $this->session->set_flashdata('pesan', 'Data User berhasil ditambah.');
        redirect('user/user_supp');
    }
    ...
    {
        $id = $this->input->post('user_id');
        $data = $this->user_m->get_supp($id)->row_array();
        header('Content-Type: application/json');
        echo json_encode($data);
    }
    ...
    public function update_supp()
    {
        $post = $this->input->post();
        $this->user_m->update_supp($post);
