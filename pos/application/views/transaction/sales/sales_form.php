                    ...
                    <div class="card-body">
                        <table width="100%">
                            <tr>
                                ...
                                <td>
                                    <div class="form-group">
                                        <input type="hidden" id="total_awal" value="" class="form-control" readonly>
                        ...
                        <tbody>
                            <?php foreach ($item as $i) { ?>
                                <tr>
                                    ...
                                    <td><?= $i->stock; ?></td>
                                    <td align="center">
                                        <button class="btn btn-xs btn-info" id="select" data-id="<?= $i->item_id; ?>" data-barcode="<?= $i->barcode; ?>" data-price="<?= $i->price; ?>" data-price_a="<?= $i->price_a; ?>" data-stock="<?= $i->stock; ?>">
...
<script>
       ...
       $.ajax({
            ...
            success: function (dataItem) {
                $.each(dataItem, function () {
                    ...
                    $('#price_a').val(dataItem.price_a);
                    $('#stock').val(dataItem.stock);
                })
            }
       }); 
    });
    ...
    $(document).on('click', '#add_cart', function() {
        ...
        var stock_update = stock - qty;
        var tot_price_a = price_a * qty;
        ...
        if (item_id == '') {
            ...
            alert('Stock tidak mencukupi, stok tinggal');
            $('#item_id').val('');
            $('#barcode').val('');
            $('#stock').val('');
            $('#price').val('');
    ...
    function calculate() {
        ...
        $('#cart_tabel tr').each(function() {
            ...
            totalawal += parseInt($(this).find('#totalawal').text(), 10);
        })
        isNaN(subtotal) ? $('#sub_total').val(0) : $('#sub_total').val(subtotal);
        isNaN(totalawal) ? $('#total_awal').val(0) : $('#total_awal').val(totalawal);
